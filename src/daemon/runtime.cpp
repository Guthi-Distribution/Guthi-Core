...
void DispatchMessage(Handle client, uint8_t *msg, uint32_t valid_length)
{
    ...
    EMessage msg_type = static_cast<EMessage>(msg[0]); // We are just consuming the first byte of the message followed immediately by length of the message 
    ...
    switch (msg_type)
    {
	    case EchoMessage : 
		    ...
		    const char* non_blocking = "Testing Non blocking read"; 
		    uint32_t message_len = strlen(non_blocking); 
		    WriteMessage(client, (const uint8_t*)non_blocking,message_len); 
		    fprintf(stderr, "Wrote the message  | %u.\n",message_len); 
		    }
		    */
		    break; 
...
uint16_t PrepareMessage(uint8_t* buf, EMessage msg_type, /* Additional data */const uint8_t* data, /* Length of additional data */ uint16_t length) {
...
int main(int argc, char **argv)
{
	...
	if (tracker.TryPopChangeInfo(changed_info)) {
		...
		const char* changed_message = "Tracked File Changed"; 
		uint16_t msg_length = PrepareMessage(msg, EMessage::TrackedFileChanged, (const uint8_t*)changed_message, strlen(changed_message)); 
